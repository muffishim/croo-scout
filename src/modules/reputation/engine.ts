...
async function reliabilityScore(
  ...
  if (orders.length === 0) return 50; // neutral default for new agents
  const completed = orders.filter((o) => o.status === "completed").length;
  return clamp((completed / orders.length) * 100);
}

async function costEfficiencyScore(
  agentId: string,
  agentPrice: number
): Promise<number> {
  try {
    ...
    // Lower price relative to median = higher efficiency
...
function popularityScore(
  ...
  // Logarithmic scale: 100 orders → ~80, 1000 orders → ~100
  const byCount = Math.min(100, Math.log10(orderCount + 1) * 40);
...
function availabilityScore(
  ...
  // Active in last 7 days → 100, 30 days → 60, 90 days → 20
...
export async function computeScoutScore(agentId: string): Promise<ScoutScore> {
  ...
  const [reliability, costEfficiency, popularity, availability] =
    await Promise.all([
      reliabilityScore(agentId, orders),
      costEfficiencyScore(agentId, minPrice || 0.1),
      Promise.resolve(popularityScore(orders)),
      Promise.resolve(availabilityScore(orders)),
    ]);

  // Benchmark performance: placeholder until benchmark engine is live
