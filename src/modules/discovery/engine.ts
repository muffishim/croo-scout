...
function keywordScore(service: MarketplaceService, queryTokens: string[]): number {
  const haystack = tokenize(`${service.name} ${service.description} ${(service.tags ?? []).join(" ")}`);
...
function toServiceSummary(s: MarketplaceService): ServiceSummary {
  return { serviceId: s.serviceId, name: s.name, price: s.price, description: s.description };
}

export async function findBestAgents(query: string, topN = 5): Promise<Recommendation[]> {
  ...
  for (const s of services) {
    ...
    if (!existing || s.price < existing.price) byAgent.set(s.agentId, s);
  }
  ...
  const candidates = await Promise.all(
    Array.from(byAgent.entries()).map(async ([agentId, service]) => {
      const scoutScore = await computeScoutScore(agentId).catch((): ScoutScore => ({
        overall: 50, grade: "C", reliability: 50, benchmarkPerformance: 50,
        costEfficiency: 50, popularity: 50, availability: 50,
      }));
